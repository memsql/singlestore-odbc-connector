...
ODBC_TEST(sql_get_diag_rec)
{
  ...
  FAIL_IF(SQLGetDiagRec(SQL_HANDLE_STMT, Stmt, 1, NULL, NULL, NULL, 0, NULL) != SQL_NO_DATA,
          "Expected no data error")
  ERR_SIMPLE_STMT(Stmt, "Some wrong query");
  sprintf(errMsg, "[ss-1.0.2][%s]You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version for the right syntax to use near 'Some wrong query' at line 1", version);
  ...
  IS_OK(CheckRec(SQL_HANDLE_DBC, Connection, 1, "01004", 0, (iOdbc() || UnixOdbc()) ? "[ss-1.0.2]String data, right-truncated" : "[Microsoft][ODBC Driver Manager] Data truncated"));
