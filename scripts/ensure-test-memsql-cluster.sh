...
DEFAULT_IMAGE_NAME="memsql/cluster-in-a-box:latest"
...
memsql-wait-start() {
  ...
  while true; do
      if mysql -u root -h 127.0.0.1 -P 5506 -ppassword -e "select 1" >/dev/null 2>/dev/null; then
          break
      fi
      echo -n "."
      sleep 0.2
...
if [[ "${EXISTS}" -eq 0 ]]; then
    ...
    docker exec -it ${CONTAINER_NAME} memsqlctl create-node --yes --port 3308 --password password
    ...
    docker exec -it ${CONTAINER_NAME} memsqlctl add-aggregator --yes --host 127.0.0.1 --port 3308 --password password
fi
...
mysql -u root -h 127.0.0.1 -P 5506 -ppassword -e 'grant all privileges on *.* to "root-ssl"@"%" require ssl with grant option'
mysql -u root -h 127.0.0.1 -P 5507 -ppassword -e 'grant all privileges on *.* to "root-ssl"@"%" require ssl with grant option'
mysql -u root -h 127.0.0.1 -P 5508 -ppassword -e 'grant all privileges on *.* to "root-ssl"@"%" require ssl with grant option'
echo "Done!"

echo
echo "Ensuring child nodes are connected using container IP"
CONTAINER_IP=$(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' ${CONTAINER_NAME})
CURRENT_LEAF_IP=$(mysql -u root -h 127.0.0.1 -P 5506 -ppassword --batch -N -e 'select host from information_schema.leaves')
if [[ ${CONTAINER_IP} != "${CURRENT_LEAF_IP}" ]]; then
    # remove leaf with current ip
    mysql -u root -h 127.0.0.1 -P 5506 -ppassword --batch -N -e "remove leaf '${CURRENT_LEAF_IP}':3307"
    # add leaf with correct ip
    mysql -u root -h 127.0.0.1 -P 5506 -ppassword --batch -N -e "add leaf root:'password'@'${CONTAINER_IP}':3307"
fi
...
if [[ ${CONTAINER_IP} != "${CURRENT_AGG_IP}" ]]; then
    ...
    mysql -u root -h 127.0.0.1 -P 5506 -ppassword --batch -N -e "remove aggregator '${CURRENT_AGG_IP}':3308"
    # add aggregator with correct ip
    mysql -u root -h 127.0.0.1 -P 5506 -ppassword --batch -N -e "add aggregator root:'password'@'${CONTAINER_IP}':3308"
fi
echo "Done!"
